...
def fetch_correlation(days=90):
    s = df_api.get_daily_candles("SBIN",  days=days)
    n = df_api.get_daily_candles("NIFTY", days=days)
...
with h1:
    ...
    st.caption(f"{dot} Yahoo Finance (NSE) • {now_ist.strftime('%d %b %Y  %H:%M:%S IST')}")
...
with h2:
    if sbin_ltp and sbin_ltp.get("ltp"):
        ...
        st.metric("SBIN", "—", "Market closed / loading…")
...
with h3:
    if nifty_ltp and nifty_ltp.get("ltp"):
        ...
        st.metric("NIFTY 50", "—", "Market closed / loading…")
...
TF_DAYS    = {"1m": 5, "5m": 10, "15m": 20, "1h": 30, "1d": 365}
...
with left:
    ...
    if not sbin_df.empty:
        ...
        st.info("SBIN chart data loading… (market may be closed on weekends)")
    ...
    if not nifty_df.empty:
        ...
        nfig.update_layout(
            ...
            margin=dict(l=0, r=0, t=30, b=0),
            xaxis_rangeslider_visible=False,
            title=dict(text=f"NIFTY 50 — {tf_label}", font=dict(size=13)),
        )
    ...
    if corr_data and corr_data.get("daily_data"):
        ...
        colors = ["rgba(63,185,80,0.5)" if r else "rgba(248,81,73,0.5)" for r in df_heat["same_dir"]]
        tips   = [
            f"{row['date']}<br>SBIN: {row['sbin_ret']:+.2f}%<br>NIFTY: {row['nifty_ret']:+.2f}%"
            for _, row in df_heat.iterrows()
        ]
        hfig = go.Figure(go.Bar(
            x=df_heat["date"], y=[1]*len(df_heat),
            marker_color=colors, hovertext=tips, hoverinfo="text",
        ))
        hfig.update_layout(
...
with right:
    ...
    # Risk levels
    st.markdown("**Risk Levels (1.5× / 2× / 3× ATR)**")
    c1, c2 = st.columns(2)
    ...
    if corr_data:
        ...
        if corr_r > 0.7:
            st.success("SBIN strongly follows NIFTY — use as leading indicator")
        elif corr_r > 0.4:
